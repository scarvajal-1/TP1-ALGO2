...
int main(int argc, char const *argv[])
{
	...
	if (strcmp(comando, "mostrar") == 0) {
		if (argc != 4) {
			fprintf(stderr,
				"Error: Número incorrecto de argumentos para 'mostrar'\n");
			print_how_to_use_program(argv[0]);
			return_code = 1;
		} else {
			if (strcmp(criterio, "nombre") == 0) {
				...
				fprintf(stderr,
					"Error: Criterio '%s' inválido para 'mostrar'. Use 'nombre' o "
					"'id'\n",
					criterio);
				print_how_to_use_program(argv[0]);
				return_code = 1;
		...
		if (argc != 5) {
			fprintf(stderr,
				"Error: Número incorrecto de argumentos para 'buscar'.\n");
			print_how_to_use_program(argv[0]);
			return_code = 1;
		} else {
			...
			if (strcmp(criterio, "nombre") == 0) {
				...
				unsigned id_buscado =
					(unsigned)atoi(valor_buscado_str);
				if (id_buscado == 0 &&
				    strcmp(valor_buscado_str, "0") != 0) {
					fprintf(stderr,
						"Error: ID inválido '%s'. Debe ser un número entero positivo\n",
						valor_buscado_str);
					return_code = 1;
				} else {
					const struct pokemon *encontrado =
				...
				tipo_pokemon tipo_buscado_enum =
					string_to_type(valor_buscado_str);

				if (tipo_buscado_enum == (tipo_pokemon)-1) {
					fprintf(stderr,
						"Error: Tipo de Pokémon inválido '%s'.\n",
						valor_buscado_str);
					return_code = 1;
				} else {
				...
				fprintf(stderr,
					"Error: Criterio '%s' inválido para 'buscar'. Use 'nombre', "
					"'id' o 'tipo'.\n",
					criterio);
				print_how_to_use_program(argv[0]);
				return_code = 1;
